...
-   ### Testing User Stories
    ...
    -   #### Viewing and Navigation
        ...
        - 404 and 500 error pages and toast messages show to keep user up to date
...
| Register user on Register page                               | Completed registration form on Register page                                                                                                              | Correctly completed input fields, clicked Register button                              | Taken to Verify Email page with Alert Message informing user that a verification email has been sent to their email                                                             | PASS      |
...
| Cafe link in What We Offer Section                           | Takes user to the Cafe page to view the cafe menu                                                                                                        | Clicked Cafe link                                                                      | Taken to cafe page                                                                                                                                                               | PASS      |
| FOOTER                                                       |                                                                                                                                                          |                                                                                        |                                                                                                                                                                                 |           |
...
| Secure checkout - incorrectly completed card details         | Shows user a message to say card details are invalid                                                                                                     | Input an incorrect card number                                                         | Message in red to advise 'Your card number is invalid'                                                                                                                          | PASS      |
| CHECKOUT SUCCESS                                             |                                                                                                                                                          |                                                                                        |                                                                                                                                                                                 |           |
...
| LOGIN                                                        |                                                                                                                                                          |                                                                                        |                                                                                                                                                                                 |           |
| Login button                                                 | Takes user to Home page with success message                                                                                                             | Correctly completed form fields and clicked Login                                      | Taken to Home page with successful sign in message                                                                                                                               | PASS      |
...
-   ### HTML Validation 
    ...
    * [Home page](documentation/testing/images/html_home.png)
...
-   ### Javascript Validation
    ...
    * [stripe js](documentation/testing/images/jshint_stripe.png) - no errors, compatible with ES6
...
-   ### WAVE Testing
    ...
    * [Booking Enquiry page](documentation/testing/images/wave_contact.png)
    ...
    During testing, it was noted that the form labels were missing from the allauth package forms. Crispy forms are used so i am unable to apply the form labels as per WAVE guidance.
- - -
...
-   ### Fixed Bugs
    ...
    * When submitting the Booking Enquiry form, the user correctly receives an email from the site confirming that the enquiry has been received. The database is also correctly populated with the Booking enquiry info and this can be view within DJango admin. However the user receives both the error and success messages. When trying to figure out why this could be i checked the Booking Enquiry view again. I wasn't sure what the issue could be. I searched and found a helpful [StackOverflow](https://stackoverflow.com/questions/1508467/log-exception-with-traceback-in-python) thread which helped me to troubleshoot the bug. I added `except Exception as e` in replacement of the else block to try to catch the error and log it in the string of the error message. Once this change was made the bug was fixed.

    * Instructor images were not rendering after deployment and hosting them within AWS. I added each instructor image via Django admin using the AWS S3 bucket url. They didn't render. I tried a number of things to try to get them to render correctly- cleared the cache, checked the permissions within AWS, checked the image URL was correct by loading the image in a new tab which it did, tried rendering the image in a paragraph outside of the instructor loop which didn't render the image. Checked the DB model to see if I had a mistake in the way the image was being stored. After a quick search of google i found a [Stack Overflow](https://stackoverflow.com/questions/8850535/how-to-get-an-imagefield-url-within-a-template) post which helped me to realize that i was trying to access the empty 'image' field rather than 'image_url', the field i was using to store the image in the database. Once accessing the image_url using `instructors.image_url` the images rendered correctly.

    * I found a bug whereby the Preschool courses, which had been rendering correctly on the Preschool courses page, were suddenly no longer rendering at all. This seemed strange as i hadn't changed any of the code relating to this feature and the other course pages which use the same view and template structure were rendering perfectly. It seemed strange that it was only preschool courses this was affecting. I thought it may have been an issue related to deploying the site as i had just done this. I re-added the data to the database and deleted the old preschool course data. This seemed to fix the issue however about 1 week later when testing the site i ran into the same bug again. This time, when having a closer look at the issue, i checked that the age group choices in the model matched the use of Preschool in the view. I noticed that Preschool file path was the only course of the four that was being passed as a lowercase which seemed to be was causing the issue. To tackle this i amended the way i filtered the courses using `courses = Course.objects.filter(age_group__iexact=age_group)` to make it case insensitive. This addressed the issue.

    * [Stack Overflow](https://stackoverflow.com/questions/41747667/bootstrap-4-responsive-tables-wont-take-up-100-width) gave me some valuable information about the responsive table design. I found that the table displaying the shopping bag contents was only responsive to around 380px. I tried to tweak padding/margin etc but user still had to scroll horizontally to view bag contents. I did not feel this was a great UX therefore i decided to add separate design for small screen sizes and hidden on medium and above. Had i had more time i think i may have removed the table completely and opted for the design i'd used for the smaller screen sizes, however the table does look smart on larger screen sizes. 

    * Spent some time figuring out a bug related to duplicate orders at checkout. The confirmation email and stripe payments appeared to be being processed as expected but a second order was made with a different order number. Slack pointed me in the direction of the shipping details which i had not included in my project. Stripe relies on shipping details to find the order and when it wasn't finding this info it was creating a new order. I therefore added shipping details into the stripe_elemts.js file and webhooks handler so stripe coud access the order information correctly. I noticed that emails were no longer sending. I removed Development=True in the Heroku config vars and this fixed the problem immediately. Stripe payments, webhooks and emails working correctly.
